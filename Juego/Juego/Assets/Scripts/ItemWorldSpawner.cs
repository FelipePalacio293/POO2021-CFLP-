...
public class ItemWorldSpawner : MonoBehaviour
{
    ...
    private void Start()
    {
        if(item.itemType == Item.ItemType.espada)
            ItemMundo.generarItemMundo(transform.position, new Espada { itemType = Item.ItemType.espada, cantidadItem = 1, esStackeable = false});
        else if (item.itemType == Item.ItemType.hacha)
            ItemMundo.generarItemMundo(transform.position, new Hacha { itemType = Item.ItemType.hacha, cantidadItem = 1, esStackeable = false });
        else if (item.itemType == Item.ItemType.lanza)
            ItemMundo.generarItemMundo(transform.position, new Hacha { itemType = Item.ItemType.lanza, cantidadItem = 1, esStackeable = false });
        ...
        else if (item.itemType == Item.ItemType.pocionHuida)
            ItemMundo.generarItemMundo(transform.position, new PocionHuida { itemType = Item.ItemType.pocionHuida, cantidadItem = 1, esStackeable = true });
        Destroy(gameObject);
    }
    ...
    {
        if (item.itemType == Item.ItemType.espada)
            ItemMundo.generarItemMundo(transformItem.position, new Espada { itemType = Item.ItemType.espada, cantidadItem = 1, esStackeable = false });
        else if (item.itemType == Item.ItemType.hacha)
            ItemMundo.generarItemMundo(transformItem.position, new Hacha { itemType = Item.ItemType.hacha, cantidadItem = 1, esStackeable = false });
        else if (item.itemType == Item.ItemType.lanza)
            ItemMundo.generarItemMundo(transformItem.position, new Hacha { itemType = Item.ItemType.lanza, cantidadItem = 1, esStackeable = false });
        else if (item.itemType == Item.ItemType.pocionVida)
            ...
            ItemMundo.generarItemMundo(transformItem.position, new PocionHuida { itemType = Item.ItemType.pocionHuida, cantidadItem = 1, esStackeable = true });
    }
}
